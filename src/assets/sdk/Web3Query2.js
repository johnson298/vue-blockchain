...
export var Web3Query2 = {
    initialize() {
        queryContract = new Web3Provider.web3.eth.Contract(QueryABI, Web3Provider.getQuery2Address());
        // window["xxxx"] = queryContract
        window["Web3Query2"] = Web3Query2
        // console.log('Web3Provider.getQuery2Address()', Web3Provider.getQuery2Address());
    },

    queryTokenList() {
        console.log('Web3Query2 queryTokenList')
        let results
        let tokenList = []
    ...
    async queryLiquidityList() {
        ...
        if (address_list && address_list.length > 0) {
            let res = await Web3Query2.QPost(Web3Provider.getOpenApiUrl()+'/market/pair/aprs', {pairIDs: pair_list});
